...
results_file="../../results/PADI_results_simple_act_test.csv"
model=simple_act
e=200
wp=0.5
...
do
    seed=$(($seed - $x))
    out_model=../../model_data/PADI_${model}model_b10_e${e}_L0.001_sMinMax_c1_k${K}_wp${wp}_ParaReLU_hSplit_seed${seed}_weightInit_test
    out_folder=../../results/PADI_${model}model_b10_e${e}_L0.001_sMinMax_c1_k${K}_wp${wp}_ParaReLU_hSplit_seed${seed}_weightInit_test
    echo $out_model

    python ../../Model/Model.py -f $train_file -v $val_file -t $test_file -i -b 10 -l 0.001 -e $e -s MinMaxScaler -c 1 -k $K -o $out_model -m $model -wp $wp -seed $seed -prefix "PADI_"
    # rm -rf $out_folder
